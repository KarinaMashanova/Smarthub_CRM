...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      { label: 'ВМР', desc: 'ВМР — заказ с маржей от 5 000 ₽. Авто-бонус 25% от маржи создаётся в БД как «Бонус / За ВМР» и пересчитывается при обновлении заказа.' },
      ...
      { label: 'ЗП день в день', desc: 'Плашка появляется только если мастер = менеджеру, заказ активный, не возврат, есть позиции и маржа не ниже 5 000 ₽. По одному заказу оплату за работу можно забрать один раз.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Фильтры', desc: 'Период, продавец, салон, тип оплаты, наличие позиций, РЦ-бонус, кредит и поиск по номеру/продавцу.' },
      { label: 'Маржа', desc: 'Маржа = выручка − себестоимость позиций. Продажи без позиций отдельно видны в фильтре и статистике.' },
      { label: 'Кредит', desc: 'Если позиция не является услугой и имеет нулевую/отрицательную выручку при наличии себестоимости — она помечается как «Кредит». Весь чек считается кредитным; смартфонные позиции из каталога (группы Android, iPhone, iPhone б/у, Смартфоны) в таком чеке тоже получают метку. Маржа по кредитным позициям с положительной выручкой считается как цена продажи − каталожная РЦ.' },
      { label: 'РЦ-бонус', desc: 'Бонус рассчитывается по аксессуарам (Аксы, Защитное стекло, Игрушки, Чехлы) из каталога. Если разница (цена продажи − каталожная РЦ) × кол-во ≥ 1 000 ₽, бонус = 50% от разницы. Кредитные и исключённые позиции не участвуют.' },
      { label: 'Бонус за наличность', desc: 'Если в продаже есть наличная оплата, маржа положительная и нет кредитных позиций, создаётся авто-бонус 2,5% от маржи.' },
      { label: 'Доступ', desc: 'Администратор видит все салоны. Менеджер видит только свои продажи.' },
    ],
  },
  {
    title: 'Касса',
    ...
    items: [
      ...
      { label: 'Сотрудник', desc: 'Необязательное поле — можно указать сотрудника, которого касается операция (например, кому выплачена зарплата или с кого удержан штраф). Штраф и Выплата Зарплаты из раздела «ЗП и бонусы» создают запись в кассе автоматически.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Авто-бонусы', desc: 'Бонусы за ВМР и наличность создаются автоматически в БД как TargetAction с source=AUTO при синхронизации заказов и продаж.' },
      { label: 'Выплата Зарплаты', desc: 'Оранжевый тип. Факт выплаты денег сотруднику из кассы. Не равна рассчитанной сумме — это отдельная операция.' },
      { label: 'Доначисление Оклада', desc: 'Синий тип. Фиксированная часть, доначисляемая к ЗП сверх бонусов.' },
      { label: 'Бонус', desc: 'Зелёный тип. Разовые премии и авто-бонусы. Авто-виды: «За ВМР» и «За наличность». Ручные виды: Отзыв, Отпускные, Работа без выходных, Выход в выходной, Выполнение плана.' },
      { label: 'Штраф', desc: 'Красный тип. Удержания. Виды: Опоздание, Списание за товар, Ревизия, Левак, Отзыв, Невыход, Работа не на потоке / курение / поведение, Неправильно заполненная продажа/заказ. Подтип НДФЛ — налог 13%.' },
      { label: 'Ремонты', desc: 'Оранжевый тип. Привязываются к заказу. «Забрал» — деньги взяты авансом и вычитаются из итога. «Не забрал» — долг компании сотруднику.' },
      { label: 'Формула', desc: 'Начислено = Бонусы + Доначисления − НДФЛ − Штрафы − Ремонты (только «Забрал»). Остаток = Начислено − Выплачено.' },
      ...
      { label: 'Источник', desc: '«Вручную» — добавлено через UI. «Импорт» — загружено скриптом. «Авто» — создано автоматически при синке заказов или продаж.' },
    ],
  },
  {
    title: 'Сотрудники',
    ...
    items: [
      ...
      { label: 'Сброс доступа', desc: 'Кнопка «Отозвать» — удаляет роль и пароль. Для выдачи нового доступа снова назначь роль в разделе «Без доступа».' },
    ],
  },
  {
    title: 'Синхронизация',
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Целевые действия', desc: 'Таблица TargetAction. Источники: MANUAL, IMPORT, AUTO. Авто-бонусы по заказам и продажам пересчитываются при обновлении данных из LiveSklad.' },
    ],
  },
]
...
const ACCESS_ROWS = [
  ...
  ['Целевые действия', 'Только свои записи', 'Все + добавление'],

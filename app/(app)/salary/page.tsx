...
const HELP_ITEMS = [
  { label: 'Типы записей', desc: 'Выплата Зарплаты (оранжевый) — факт выплаты из кассы. Доначисление Оклада (синий) — фиксированная добавка к ЗП. Бонус (зелёный) — разовые премии. Штраф (красный) — удержания.' },
  { label: 'Бонус за ВМР', desc: 'Авторасчёт: при марже заказа от 5 000 ₽ создаётся бонус 25% от маржи. Помечается тегом «ВМР».' },
  { label: 'Ремонты', desc: 'Сотрудник привязывает оплату ремонта к заказу. «Забрал» — деньги взяты авансом и вычитаются из итога ЗП. «Не забрал» — долг компании сотруднику.' },
  { label: 'Формула Итого', desc: 'Итого ЗП = Бонусы + Доначисления − НДФЛ − Штрафы − Ремонты (только «Забрал»). Остаток = Итого − Выплачено.' },
  { label: 'Источник', desc: '«Вручную» — создано через интерфейс. «Импорт» — загружено скриптом. «Авто» — создано автоматически при синке заказов.' },
  { label: 'Права доступа', desc: 'Менеджер видит только свои записи и может добавлять ремонты. Создавать другие типы — только администратор.' },
]
